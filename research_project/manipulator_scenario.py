...
DynamicCuboid(
    prim_path="/World/red_cube", # The prim path of the cube in the USD stage
    name="red_cube", # The unique name used to retrieve the object from the scene later on
    position=np.array([-0.3, -0.3, 0.05526]), # Using the current stage units which is in meters by default.
    scale=np.array([0.05, 0.05, 0.05]), # most arguments accept mainly numpy arrays.
    color=np.array([1.0, 0, 0]), # RGB channels, red
    )

# DynamicCuboid(
#     prim_path="/World/yellow_cube", # The prim path of the cube in the USD stage
#     name="yellow_cube", # The unique name used to retrieve the object from the scene later on
#     position=np.array([0.3, 0.4, 0.05526]), # Using the current stage units which is in meters by default.
#     scale=np.array([0.0515, 0.0515, 0.08]), # most arguments accept mainly numpy arrays.
#     color=np.array([1.0, 1.0, 0]), # RGB channels, yellow
#     )

# DynamicCuboid(
#     prim_path="/World/green_cube", # The prim path of the cube in the USD stage
#     name="green_cube", # The unique name used to retrieve the object from the scene later on
#     position=np.array([0.4, 0.3, 0.05526]), # Using the current stage units which is in meters by default.
#     scale=np.array([0.0515, 0.0515, 0.08]), # most arguments accept mainly numpy arrays.
#     color=np.array([0, 1.0, 0]), # RGB channels, green
#     )
